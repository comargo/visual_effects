...
void fadeUsingColor( RGB* leds, uint16_t numLeds, RGB colormask)
{
    for( uint16_t i = 0; i < numLeds; ++i) {
      leds[i] = (RGB){{
        scale8(leds[i].r, colormask.r),
        scale8(leds[i].g, colormask.g),
        scale8(leds[i].b, colormask.b)
      }};
    }
}
...
RGB ColorFromPalette32( const RGB *pal, uint8_t index, uint8_t brightness, TBlendType blendType)
{
    uint8_t hi5 = index;
    hi5 >>= 3;
...
RGB ColorFromPalette256( const RGB *pal, uint8_t index, uint8_t brightness, TBlendType blendType)
{
...
{
  return (RGB){{
      applyGamma_video(orig.r, gamma),
      applyGamma_video(orig.g, gamma),
      applyGamma_video(orig.b, gamma)
  }};
}
...
{
  return (RGB){{
      applyGamma_video(orig.r, gammaR),
      applyGamma_video(orig.g, gammaG),
      applyGamma_video(orig.b, gammaB)
  }};
}
